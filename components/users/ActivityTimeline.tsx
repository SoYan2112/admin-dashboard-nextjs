    ...
    return (
        <div className="space-y-4">
            ...
            <div className="border rounded-lg divide-y bg-white overflow-hidden">
                {logs.length === 0 && <p className="p-4 text-gray-500 italic">No activity yet.</p>}
                
                {logs.map((log) => (
                    <div key={log.id} className="p-4 hover:bg-slate-50 transition-colors">
                        <div className="flex justify-between items-start">
                            {/* LOGIC ĐỔI MÀU Ở ĐÂY */}
                            <p className={`font-bold uppercase text-[10px] tracking-widest px-2 py-0.5 rounded border ${
                                log.action === 'DELETE' 
                        ...
                        <p className="text-sm mt-2 text-slate-700 font-medium">{log.details}</p>
